...
## Selfhost Fork Integration

- Follow `docs/selfhost-updates.md` and retain the capabilities in `selfhost/patches.json`.
- Before promotion, run `scripts/check-selfhost-integration.sh --current <deployed-commit>`. Preserve accepted patch ancestry through non-rewriting merges.
- Local deployment and public releases are separate. Do not create tags or releases without explicit owner authorization, or restart the daemon hosting the current task.
